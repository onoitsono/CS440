      ...
      <div class='span9' >
        ...
        <!-- Main content and middle sidebar -->
          <div class='span12'>
            ...
            <!-- Main Content -->
            <h2 class="title" id="page-title"><a href="acct.php">Submit Another Request</a></h2>
...
<?php
	...
	$formSubmission = "acct.php";
	
	if (isset($_POST['requestor-mail']) && isset($_POST['acct']) && 
	isset($_POST['dp-super-email']) && isset($_POST['dp-dpt']) && isset($_POST['date']))
	{
		$to = 'cn.help@oregonstate.edu';
		$subject = 'IGNORE Account Removal Page Testing IGNORE';
		$emailRmv = $_POST['email-rmv'];
		$networkRmv = $_POST['p-delete'];
		
		$mailResult = "<strong>Requester Email: </strong>" . $_POST['requestor-mail'] . "<br>";
		$mailResult .= "<strong>Account to Terminate: </strong>" . $_POST['acct'] . "<br>";
		$mailResult .= "<strong>Departing Employee's Supervisor Email: </strong>" . $_POST['dp-super-email'] . "<br>";
		$mailResult .= "<strong>Employee Dept: </strong>" . $_POST['dp-dpt'] . "<br>";
		$mailResult .= "<strong>Removal Date: </strong>" . $_POST['date'] . "<br><br>";
		$mailResult .= "<strong>Special Needs: </strong><br>";
		
		if (isset($_POST['special'])){
			foreach($_POST['special'] as $sp){
				$mailResult .= $sp . "<br>";
			}
		} else {
			$mailResult .= "none<br>";
		...
		$mailResult .= "<br>";
		$mailResult .= "<strong>Email Removal: </strong>" . $emailRmv . "<br>";
		$mailResult .= "<strong>Additional Info: </strong>";
		if (isset($_POST[$emailRmv])){
			$mailResult .= $_POST[$emailRmv];
		} else {
			$mailResult .= "none";
		}
		
		$mailResult .= "<br><br>";
		$mailResult .= "<strong>Network Space Removal: </strong>" . $networkRmv . "<br>";
		$mailResult .= "<strong>Additional Info: </strong>";
		if (isset($_POST[$networkRmv])){
			$mailResult .= $_POST[$networkRmv];
		} else {
			$mailResult .= "none";
		}
		
		$mailResult .= "<br><br>";
		$mailResult .= "<strong>Additional Comments:</strong><br>";
		if (isset($_POST['add-comments']) && $_POST['add-comments'] != ''){
			$mailResult .= $_POST['add-comments'];
		} else {
			$mailResult .= "none";
		}
		
		$headers =  "MIME-Version: 10\r\n";
		$headers .= 'From: ' . ' <' . $_POST['requestor-mail'] . '>' . "\r\n";
		...
		// Redirect back to form if brought here w/o proper post variables.
		echo "<script>window.location='" . $formSubmission . "';</script>";
	}
?>
<div id='success' class='alert'>SUCCESSFUL FORM SUBMISSION</div>
<div class="content">
<?php
	echo $mailResult . '<br>';
?>
